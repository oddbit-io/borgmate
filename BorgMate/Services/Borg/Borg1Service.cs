...
public class Borg1Service(ILogger<Borg1Service> logger, AppSettings settings, SshAgentHelper sshAgent, WslHelper wsl) : BorgServiceBase(logger, settings, sshAgent, wsl)
{
        ...
        CancellationToken ct = default)
    {
        ...
        var args = $"init --encryption {repo.EncryptionMode.ToBorgString()} \"{P(repo.Path)}\"";
        ...
        Action<string>? onStderrLine = null)
    {
        ...
        var rateLimit = repo.RateLimit > 0 ? $" --remote-ratelimit {repo.RateLimit}" : "";
    ...
    {
        var env = await BuildEnvironmentAsync(repo);
        var args = $"list --json \"{P(repo.Path)}\"";
        return await RunCommandAsync(BorgBinary, args, env, ct: ct);
    }
    ...
    {
        var env = await BuildEnvironmentAsync(repo);
        var args = $"delete \"{P(repo.Path)}::{archiveName}\"";
        return await RunCommandAsync(BorgBinary, args, env, ct: ct);
    }
        ...
        CancellationToken ct = default)
    {
        ...
        var args = $"prune --stats --list{retention} \"{P(repo.Path)}\"";
        return await RunCommandAsync(BorgBinary, args, env, ct: ct);
    }
    ...
    {
        var env = await BuildEnvironmentAsync(repo);
        var args = $"check --progress \"{P(repo.Path)}\"";
        return await RunCommandAsync(BorgBinary, args, env, ct: ct, onStderrLine: onStderrLine);
    }
    ...
    {
        var env = await BuildEnvironmentAsync(repo);
        var args = $"compact --progress \"{P(repo.Path)}\"";
        return await RunCommandAsync(BorgBinary, args, env, ct: ct, onStderrLine: onStderrLine);
    }
    ...
    {
        var env = await BuildEnvironmentAsync(repo);
        var args = $"info --json \"{P(repo.Path)}\"";
        return await RunCommandAsync(BorgBinary, args, env, ct: ct);
    }
